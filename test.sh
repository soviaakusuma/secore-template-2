...
return_status() {
  ...
  if [ $status -ne 0 ] && [ "`uname`" = Darwin ]; then
    ...
    echo "  docker-compose run --rm testsql psql $(. .env && echo $APP_POSTGRES_DATABASE)"
  fi
...
# start postgres
docker-compose --no-ansi up -d postgres
...
timeout 60 docker-compose exec -Tu postgres postgres sh -c "until psql -h 127.0.0.1 -U \"\$POSTGRES_USER\" \"\$POSTGRES_DB\" -ec '\q'; do echo 'Postgres is unavailable - sleeping'; sleep 1; done" </dev/null
...
VERSION=${INITVERSION:-latest} GROW=init timeout 60 docker-compose --no-ansi up --exit-code-from $app $app
if [ "$INITVERSION" ]; then
  expectedVer=$(awk -F. '{printf "%.2d%.2d%.3d\n", $1, $2, $3}' <<<"$INITVERSION")
  dbVer=$(docker-compose run -T --rm testsql psql "$(. .env && echo $APP_POSTGRES_DATABASE)" -qAtc 'select version from grow.log')
...
# run sql tests
TIMEOUT=300
COMPOSE_HTTP_TIMEOUT=$(($TIMEOUT*2)) timeout $TIMEOUT docker-compose --no-ansi up --exit-code-from testsql testsql
